...
void Heuristic::step(Ruffle& ruffle) {
	for (auto section = ruffle.sections.begin(); section != ruffle.sections.end(); ++section) {
		...
		//real closest_distance = infinity;
		//int closest_vertex = -1;
		real segment_distance = 0;
		for (auto& seg : section->mesh_segments) {
			auto vertex = seg->start;
			Vector2 pos = ruffle.simulation_mesh.get_vertex_position(*vertex);
			real distance = target.signed_distance(pos);
			dbg(distance);
		...
		dbg(segment_distance);

		if (std::isfinite(segment_distance)) {
			section->length += 0.05 * segment_distance;
		}
	}
...
//
//		if (std::isfinite(segment_distance)) {
//			//section->length += 0.1 * segment_distance;
//			double step_percent = 0.05;
//			section->length = segment_distance > 0 ? section->length * (1. + step_percent) : section->length * (1. - step_percent);
//		}
//	}
//
//	for (auto section = ruffle.sections.begin(); section != ruffle.sections.end(); ++section) {
